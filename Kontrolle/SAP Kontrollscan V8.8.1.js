// ==UserScript==
// @name         SAP Kontrollscan V8.8.1 (cleanup/teardown + SPA-safe)
// @namespace    local.sap.kontrollscan.stop
// @version      8.8.1
// @description  v8.8.0 + P0: full teardown (timers/observer/listeners/UI/audio) on pagehide/beforeunload + anti-double-init guard for SPA/soft navigation. Light cleanup on WHITE/YELLOW.
...
(function () {
  ...
  try {
    if (window[GUARD_KEY] && typeof window[GUARD_KEY].teardown === 'function') {
      // If Tampermonkey injected again (SPA / soft nav), fully teardown previous instance
  ...
  // handlers (so we can remove listeners)
  const HANDLERS = {
    onUserBusy: null,
  ...
  function ensureSettingsPanel() {
    ...
    // IMPORTANT: store handler so we can remove on teardown
  ...
  function ui5LoadMore(tableCtrl) {
    ...
    try {
      ...
      // keep existing fallback (as in v8.8.0) — not part of teardown scope
  ...
  function lightCleanupOnLeaveHu(baseState) {
    // Stop “heavy” background activity immediately when leaving HU
    stopRamp();
    detachObserver();
    stopPulseSync();
    ...
    // Keep lastErrorSignature (anti-flicker) as before; teardown handles full reset.
  }
  ...
  function teardown(reason = 'teardown') {
    ...
    // close audio to avoid any resource tail in SPA / BFCache
    closeAudioCtx();
