// ==UserScript==
// @name         SAP Kontrollscan V8.8.3
// @namespace    local.sap.kontrollscan.stop
// @version      8.8.3
// @description  v8.8.2 + FIX: beep only when errors are new/increased (or replaced with same count). No beep when activeCount decreases but errors still exist.
...
(function () {
  ...
  // ✅ FIX: можно обновлять signature всегда, но "заводить" beep только если allowBeep=true
  ...
  function lightCleanupOnLeaveHu(baseState) {
    ...
    // ✅ reset счётчика при уходе со страницы/не HU
  ...
  function tick() {
    ...
    if (hasError) {
      ...
      // ✅ FIX: НЕ пищим, если ошибок стало меньше, но ещё есть ошибки
      // ✅ Пищим, если:
      //   - сигнатура изменилась И (ошибок стало больше ИЛИ ошибок столько же -> возможна "замена" ошибок)
      ...
      // ✅ когда ошибок нет — сбрасываем счётчик, чтобы следующий 0->1 снова дал звук
      lastActiveErrorCount = 0;
      lastErrorSignature = '';
      lastErrorAt = 0;
    }

    // ✅ обновляем "предыдущее" значение в конце тика
