...
// @version      9.0.2
// @description  v9.0.2 + FIX: beep only when errors are new/increased (or replaced with same count). + FIX: stable row keys (no re-beep when row indices shift). + FIX: reset Weitere burst limiter on HU change / timeout.
// @match        https://vhfiwp61ci.sap.ugfischer.com:44300/*
// @grant        none
// @updateURL    https://raw.githubusercontent.com/Skrap87/SAP-Helper/main/Kontrolle/main/SAP_Kontrollscan_main.js
// @downloadURL  https://raw.githubusercontent.com/Skrap87/SAP-Helper/main/Kontrolle/main/SAP_Kontrollscan_main.js
// ==/UserScript==
...
(function () {
  ...
  // ✅ FIX: стабильный ключ строки, чтобы не пищать повторно при сдвиге индексов (когда строки исчезают)
  function stableRowKey(tr) {
  if (!tr) return 'row:null';

  // 1) Самое надёжное: взять Material из конкретной колонки (у тебя aria-colindex="3")
  ...
  if (matCell) {
    ...
    for (const span of spans) {
      ...
      // Материал: только цифры, допускаем 3+ знака (3/6/7 и т.д.)
      if (/^\d{3,}$/.test(txt)) return `MAT:${txt}`;
    }
  }

  // 2) fallback: любой .sapMText в строке, но тоже 3+ цифры
  const textSpans = tr.querySelectorAll('.sapMText');
  ...
  // ✅ NEW: чтобы не пищать при уменьшении количества ошибок, пока ещё есть ошибки
  let lastActiveErrorCount = 0;
  ...
  function setPendingBeepOnSignatureChange(sig, allowBeep) {
    if (!sig) return;
  ...
  function tick() {
    ...
    if (hasError) {
      ...
      const signatureChanged = (sig !== lastErrorSignature);
      const allowBeep = signatureChanged && (activeCount >= lastActiveErrorCount);
